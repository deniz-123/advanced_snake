...
class SnakeGameAI():
    ...
    def restart(self):
        ...
        self.enemy = Enemy(self.NODE_HEIGHT, self.NODE_WIDTH, self.ORANGE, [-100000,100000])
    ...
    def collision(self, get_danger = False, danger_dir = [0,0]):
        # snake and enemy
        snake = self.snake
        enemy = self.enemy
        # positions
        ...
        if get_danger:
            if danger_dir == snake_dir:   # STRAIGHT
                if snake_dir == self.UP:  # Snake going UP
                    ...
                    min_y = snake_pos[1] - (1+len(snake.nodes))*snake.height
                    max_y = snake_pos[1]
                    ...
                    for tail in snake.nodes:
                        if min_x <= tail.position[0] <= max_x and min_y <= tail.position[0] <= max_y:
                            return True
                    # check if enemy snake is in front
                    if min_x <= enemy_pos[0] <= max_x and min_y <= enemy_pos[0] <= max_y:
                        return True
                    ...
                    max_x = snake_pos[0] + (2+len(snake.nodes))*snake.width
                    min_y = snake_pos[1] 
                    ...
                    max_y = snake_pos[1] + (2+len(snake.nodes))*snake.height
                    ...
                    min_x = snake_pos[0] - (1+len(snake.nodes))*snake.width
                    max_x = snake_pos[0]
                    min_y = snake_pos[1] 
                    max_y = snake_pos[1] + snake.height
                ...
                if danger_dir == self.LEFT and snake_dir == self.DOWN:  # Snake going DOWN
                    min_x = snake_pos[0] - (1+len(snake.nodes))*snake.width
                    max_x = snake_pos[0]
                    min_y = snake_pos[1] - (len(snake.nodes))*snake.height
                    max_y = snake_pos[1] + snake.height
                    ...
                    max_x = snake_pos[0] + (2+len(snake.nodes))*snake.width
                    min_y = snake_pos[1]
                    max_y = snake_pos[1] - (1+len(snake.nodes))*snake.height
                    ...
                    min_x = snake_pos[0]
                    max_x = snake_pos[0] - (1+len(snake.nodes))*snake.width
                    min_y = snake_pos[1]
                    max_y = snake_pos[1] - (1+len(snake.nodes))*snake.height
                    ...
                    min_x = snake_pos[0] - (len(snake.nodes))*snake.width
                    max_x = snake_pos[0] + snake.width
                    min_y = snake_pos[1] + snake.height
                    max_y = snake_pos[1] + (2+len(snake.nodes))*snake.height
                ...
                if danger_dir == self.RIGHT_UP and snake_dir == self.UP: # Snake going UP
                    ...
                    # check if any edge inside the area
                    if min_x >= self.WIDTH or max_x >= self.WIDTH or min_y <= 0 or max_y <= 0:
                        return True
                    ...
                    # check if any edge inside the area
                    if min_x >= self.WIDTH or max_x >= self.WIDTH or min_y >= self.HEIGHT or max_y >= self.HEIGHT:
                        return True
                    # check if any body inside the area
                    for tail in snake.nodes:
                        if min_x <= tail.position[0] <= max_x and min_y <= tail.position[0] <= max_y:
                    ...
                    # check if any edge inside the area
                    if min_x <= 0 or max_x <= 0 or min_y >= self.HEIGHT or max_y >= self.HEIGHT:
                        return True
                    # check if any body inside the area
                    for tail in snake.nodes:
                        if min_x <= tail.position[0] <= max_x and min_y <= tail.position[0] <= max_y:
